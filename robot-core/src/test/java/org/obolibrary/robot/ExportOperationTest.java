...
public class ExportOperationTest extends CoreTest {
  ...
  /**
   * Test exporting all new named headings using all-axioms ontology.
   *
   * @throws Exception on any problem
   */
  @Test
  public void testExportNewHeadings() throws Exception {
    OWLOntology ontology = loadOntology("/axioms.owl");
    IOHelper ioHelper = new IOHelper();
    //    ioHelper.addPrefix(
    //        "simple",
    ...
    // export classes
    Map<String, String> options = ExportOperation.getDefaultOptions();
    options.put("include", "classes properties individuals");
    ...
    // column header labels should match
    for (short colIx = minColIx; colIx < maxColIx; colIx++) {
      org.apache.poi.ss.usermodel.Cell cell = header.getCell(colIx);
      if (cell == null) {
        continue;
      }
      String foundHeader = cell.getStringCellValue();
